...
/**
 * A template's <section> element becomes SectionMetadata, which is not a
 * FieldMetadata and holds its own children via getItems(). Regression coverage
 * for the bug where those children were silently dropped instead of being
 * reported (either directly or nested, or inside a section).
 */
...
final class TemplatesResourceSectionTest extends TestCase
{
    public function testFlattensSectionFieldsIntoParentFieldList(): void
    {
        ...
        // The section itself never appears as a field — only its writable children do.
    ...
    {
        // Ref-based block type, resolved from the global block registry — same
        // mechanism the box.xml / homepage.xml real-world templates rely on.
    ...
    public function testDeeplyNestedAcyclicSectionsAreDepthCapped(): void
    {
        $form = new FormMetadata();
        $form->setKey('default');
        // Deeper than TemplatesResource's depth cap (20), but not cyclic — a
        // pathological chain of distinct sections, not a repeated reference.
        ...
        // Truncated before the leaf field is ever reached — result stays a single marker.
        $this->assertCount(1, $fields);
    ...
    /**
     * Builds a chain of $depth nested sections, with a single "leaf" text field
