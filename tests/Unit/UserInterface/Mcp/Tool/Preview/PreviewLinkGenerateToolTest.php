...
final class PreviewLinkGenerateToolTest extends TestCase
{
    ...
    {
        if ('page' === $type) {
            $entity = (new Page())->setWebspaceKey('example');
            $this->pageRepository->getOneBy(Argument::cetera())->willReturn($entity);
        } else {
            $entity = new Article();
            $this->articleRepository->getOneBy(Argument::cetera())->willReturn($entity);
    ...
    public function testTypeIsMappedToResourceKeyForGenerate(): void
    {
        ...
        $previewLink = new PreviewLink('tok', 'articles', 'article-uuid-1', 'en', []);
    ...
    public function testGeneratePreviewLinkPassesWebspaceInOptions(): void
    {
        ...
        $previewLink = new PreviewLink('tok', 'pages', 'uuid-1', 'en', []);
