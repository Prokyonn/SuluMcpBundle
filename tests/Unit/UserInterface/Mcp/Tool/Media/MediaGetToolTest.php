...
final class MediaGetToolTest extends TestCase
{
    ...
    /**
     * Once any method is stubbed on a Prophecy double, every other method the
     * code under test calls on it must be stubbed too (unstubbed calls on an
     * otherwise-untouched double are allowed, but a partially-stubbed double
     * is not) -- these tests only care about the permission check, not the
     * detail fields getMedia() also reads to build its result array.
     *
