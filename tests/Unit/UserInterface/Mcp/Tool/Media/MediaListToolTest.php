...
final class MediaListToolTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tool = new MediaListTool($this->mediaManager->reveal(), $this->permissionChecker->reveal(), $this->systemCollectionManager->reveal());
    }
    ...
    {
        $this->permissionChecker->has(Argument::cetera())->willReturn(false);
    ...
    public function testListMediaDoesNotExcludeSystemCollectionsWhenPermitted(): void
    {
        ...
        $tool = new MediaListTool($mediaManager->reveal(), $permissionChecker->reveal(), $this->systemCollectionManager->reveal());
    ...
    public function testListMediaWithCollectionIdChecksObjectPermission(): void
    {
        ...
        $this->permissionChecker
            ->check('sulu.media.collections', PermissionTypes::VIEW, 'en', Collection::class, 5)
            ->shouldBeCalledOnce();
    ...
    public function testListMediaWithoutCollectionIdFiltersRowsByCollectionPermission(): void
    {
        ...
        $this->mediaManager->get(Argument::cetera())->willReturn([$mediaInAllowedCollection->reveal(), $mediaInDeniedCollection->reveal()]);
        ...
        $this->permissionChecker->has('sulu.media.system_collections', Argument::cetera())->willReturn(true);
        $this->permissionChecker->has('sulu.media.collections', PermissionTypes::VIEW, 'en', Collection::class, 3)->willReturn(true);
        $this->permissionChecker->has(Argument::cetera())->willReturn(false);
    ...
    public function testListMediaWithoutCollectionIdExcludesSystemCollectionMediaWhenSystemViewDenied(): void
    {
        ...
        $this->mediaManager->get(Argument::cetera())->willReturn([$mediaInNormalCollection->reveal(), $mediaInSystemCollection->reveal()]);
        $this->mediaManager->getCount()->willReturn(2);

        // Caller has collection EDIT on both collections, but NOT system_collections VIEW.
        $this->permissionChecker->has('sulu.media.system_collections', Argument::cetera())->willReturn(false);
        $this->permissionChecker->has(Argument::cetera())->willReturn(true);
